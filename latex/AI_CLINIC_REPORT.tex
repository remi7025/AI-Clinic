...
\newcommand{\SubmissionDate}{30/08/2026}
...
\vspace*{0.4cm}
\includegraphics[width=0.48\textwidth]{assets/aivancity_logo.png}
...
{\color{aivatext}\normalsize
\begin{tabular}{@{}>{\bfseries}p{4.5cm} p{\dimexpr\linewidth-4.7cm\relax}@{}}
Team & Remi Uttejitha ALLAM \\
                       & Baptiste Langlois \\
                       & Tong Li \\
                       & Darryl Towa \\
...
\begin{enumerate}[label=(\roman*), leftmargin=*, itemsep=2pt, topsep=4pt]
  \item a structured literature review of AI healthcare compliance across 19 countries in seven regions;
  \item a curated dataset scoring seven regulatory themes on a 1--10 scale;
  \item an interactive dashboard for geospatial comparison, theme analysis, global trends, use-case readiness, and literature browsing.
\end{enumerate}
...
\textbf{Keywords:} AI healthcare regulation; Software as a Medical Device (SaMD); General Data Protection Regulation (GDPR); compliance dashboard; data privacy; clinical validation; EU AI Act; post-market surveillance.
...
The project aims to:
\begin{enumerate}
  \item Synthesise peer-reviewed and policy literature on AI healthcare regulation across 19 countries;
...
\paragraph{How to read the risk classes.}
The table lists how selected jurisdictions \emph{label} medical devices (including AI software as a medical device, SaMD) into risk tiers. The class names differ by country, but the principle is the same worldwide: \textbf{higher class = higher potential harm if the device fails = stricter approval and post-market oversight}. Classification is based on \textbf{intended use} and \textbf{patient risk}---not on how ``advanced'' the algorithm is.
...
\begin{itemize}
  \item \textbf{Class I (lowest risk):} low impact if wrong; often general controls only (labelling, quality). Many US Class~I devices are exempt from premarket submission; many EU Class~I products can be self-declared for CE marking.
  \item \textbf{Class II / EU IIa--IIb (moderate risk):} more clinical evidence and regulatory review than Class~I. In the US, Class~II often follows a 510(k) or De Novo pathway. The EU splits medium risk into \textbf{IIa} (lower--moderate) and \textbf{IIb} (moderate--higher, closer to Class~III).
  \item \textbf{Class III (highest risk in three-tier systems):} life-sustaining, life-supporting, or high harm if wrong; strongest premarket evidence (e.g.\ FDA PMA, EU Class~III with full notified-body review).
  \item \textbf{Class IV (Canada and Japan):} highest tier in four-class systems; comparable role to Class~III elsewhere.
\end{itemize}

For AI/SaMD, illustrative examples are: administrative or low-risk wellness software (Class~I or non-device); AI that \emph{flags} findings for clinician review (often Class~II / EU IIa); AI that \emph{strongly drives} diagnosis in serious disease (Class~II--III / EU IIb--III); autonomous critical treatment decisions (typically Class~III / EU III). These classes feed the \emph{approval process} theme in the project dataset; they are product-risk labels, distinct from the jurisdiction-level maturity scores used elsewhere in this report.
...
\begin{itemize}
  ...
  \item \textbf{Advanced} --- dedicated AI/ML SaMD pathways or high-risk AI law, substantial device authorisations, and lifecycle governance (e.g.\ United States, EU, United Kingdom, Japan, Singapore).
\end{itemize}
...
\begin{enumerate}
  \item Scope 19 jurisdictions across seven regions.
...
Table~\ref{tab:overall-rank} ranks all jurisdictions by the arithmetic mean of the seven theme scores. The European Union and the United States occupy the top tier (overall $\geq 7.6$). Kenya and Nigeria sit at the bottom of the sample (overall $\approx 2.7$--$2.9$), illustrating the capacity gap between advanced regulators and early-stage systems.

\input{generated/overall_rank.tex}
...
\begin{figure}[!htbp]
  ...
  \caption[Overall country ranking]{\textbf{Overall country ranking.} Each bar is the mean of the seven theme scores (1--10) for one jurisdiction. Bars are coloured by region. Longer bars mean stronger documented AI healthcare compliance. Read from bottom (highest) to top (lowest): the EU and the United States lead; Kenya and Nigeria trail. Use this figure to show the jury the global spread of regulatory maturity at a glance.}
...
\input{generated/country_scores.tex}
}
...
\input{generated/theme_leaders.tex}
...
\input{generated/regional_means.tex}
...
\subsection{Europe}
The EU, the United Kingdom, and Switzerland form the densest high-compliance cluster. The EU AI Act adds a binding high-risk layer on MDR SaMD and GDPR, lifting ethics and transparency relative to voluntary NIST-style frameworks \cite{euaiact2024}. Europe is the reference region for rights-based AI healthcare governance.
...
\begin{figure}[!htbp]
  ...
  \caption[Radar profiles of the top five scorers]{\textbf{Radar profiles of the top five scorers:}\\ Each coloured polygon is one top jurisdiction across the seven themes (axes from centre 0 to outer rim 10). A wider polygon means stronger scores. Compare shapes: the EU typically extends farthest on privacy and ethics; the United States extends farthest on clinical validation and approval process, with a relative dip on transparency/ethics. This shows that ``best country'' depends on which theme matters for the product.}
...
The following profiles, ordered by overall score, summarise each jurisdiction. Each country is presented with highlighted headings and stacked fields for quick reading. They mirror the dashboard Country Details page and should be read with Tables~\ref{tab:overall-rank} and~\ref{tab:country-scores}.

\input{generated/country_profiles.tex}
...
\begin{enumerate}
  ...
  \item \textbf{Europe and North America dominate the overall ranking}, with the EU strongest on enforceable ethics and transparency, and the United States strongest on clinical validation and device volume.
...
\paragraph{For developers and product teams:}
...
\paragraph{For policymakers and regulators:}
...
\paragraph{For researchers, educators, and students:}
...
\paragraph{Future development of the platform:}
